...
class Solution{
    ...
    private HashMap<String, List<Node>> subtrees = new HashMap<>();
    ...
    private String serializeTree(Node root) {
        ...
        subtrees.computeIfAbsent(current, k -> new ArrayList<>()).add(root);
    ...
    public List<Node> printAllDups(Node root) {
        ...
        for (Map.Entry<String, List<Node>> entry : subtrees.entrySet()) {
            if (entry.getValue().size() > 1) {
                result.add(entry.getValue().get(0));
            }
        }

        result.sort(Comparator.comparingInt(a -> a.data));
